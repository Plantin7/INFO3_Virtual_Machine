...
public class StackInterpreter {
	...
	public static Object execute(JSObject function, Dictionary dict, JSObject globalEnv) {
		...
		for (var i = 0; i < code.slotCount(); i++) { // Toutes les variables locales sont initialisés a UNDEFINED
			stack[i] = undefined;
		}
		...
		for (;;) {
			switch (instrs[pc++]) {
				...
				case Instructions.LOAD -> { // prendre des choses sur la pile et les mettre dans des variables locales
				...
				case Instructions.STORE -> { // prendre des trucs des variables locales et stoker sur la pile
				...
				case Instructions.FUNCALL -> {
					...
					// find baseArg
					var baseArg = sp - argumentCount; // get the first argument
				...
				case Instructions.RET -> {
					...
					pc = stack [activation + PC_OFFSET]; // + 0 : fct  // + 1 bp // +2 pc
				...
				case Instructions.NEW -> {
					...
					var clazz = (JSObject) decodeDictObject(vClass, dict); // decode from dict
					...
					// le tas est full lorsque la taille de creation d'un nouvelle
                    // objet n'est pas suffisant pour l'accueillir
				...
				case Instructions.GET -> {
                    ...
                    var value = heap[ref + OBJECT_HEADER_SIZE + (int) slot];
					push(stack, sp++, value);
				}
